 ...
 *
 * This script verifies that the subscription limit implementation is correct.
 * Run with: npx ts-node tests/verify-limits.ts
 */
...
function getFamilyTreeLimit(plan: SubscriptionPlan): number {
  switch (plan) {
    case "pro":
...
function getMembersPerTreeLimit(plan: SubscriptionPlan): number {
  switch (plan) {
    case "pro":
...
function canCreateFamilyTree(currentCount: number, plan: SubscriptionPlan): boolean {
  const limit = getFamilyTreeLimit(plan);
  if (isUnlimited(limit)) return true;
  return currentCount < limit;
}

function canAddMember(currentCount: number, plan: SubscriptionPlan): boolean {
...
function getRemainingSlots(currentCount: number, plan: SubscriptionPlan, type: 'tree' | 'member'): number | 'unlimited' {
...
function generateLimitError(plan: SubscriptionPlan, type: 'tree' | 'member', limit: number): string {
  if (type === 'tree') {
